...
namespace JAD {
  ...
  class JSONLexer : public jsonFlexLexer
  {
    Buffer* buffer;
  ...
  JSONLexer::JSONLexer(Buffer* buf) : jsonFlexLexer(0,0)
  {
    buffer = buf;
  }
  ...
  void uc_encode_json(const UniversalContainer& uc, Buffer* buffer)
  {
    ...
    case uc_Character :
      ...
      break;
    default :      
      uc_encode_json(UniversalContainerAdapter[uc.get_type()]->pack(uc.cast_or_throw(uc.get_type())), buffer);
    } //end type switch
  }

  void uc_encode_pretty_json(const UniversalContainer& uc, Buffer* buffer, 
                             unsigned indent, unsigned depth)
  {
    ...
    case uc_Character :
      ...
      break;
    default :    
      uc_encode_pretty_json(UniversalContainerAdapter[uc.get_type()]->pack(uc.cast_or_throw(
      uc.get_type())), buffer, indent, depth);
      break;
    } //end type switch
  ...
  Buffer* uc_encode_pretty_json(const UniversalContainer& uc, unsigned indent)
  {
    Buffer* buffer = new Buffer;
    uc_encode_pretty_json(uc, buffer, indent, 0);
    buffer->put('\0');
    buffer->length--;
    buffer->rewind();
    return buffer;
